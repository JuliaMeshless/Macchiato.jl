...
combine_surfaces!(part, :surface1, :surface2, :surface4)
...
if structured
    ...
    noderepel!(cloud, ConstantSpacing(Δ[1]); α = Δ[1] / 30, max_iters = 1e3)
end
...
function update_velocity!(u_star, u, domain, dt, ∇², ∂)
...
function mystep!(u, u_old, domain, dt, ∇², ∂, c)
    # calculate intermediate velocity
    update_velocity!(u, u_old, domain, dt, ∇², ∂)
...
function run_sim(u, u_old, domain, ∇², ∂, c, dt; tol = 1e-5, max_steps = 10_000)
    i = 0
    prog = ProgressUnknown(desc = "Working hard:", spinner = true)
    conv = IncompressibleFluid(Inf, Inf, Inf)
    while (conv.u > tol) || (conv.v > tol)
        next!(prog)
        copyto!(u_old, u)
        mystep!(u, u_old, domain, dt, ∇², ∂, c)
        if i > max_steps

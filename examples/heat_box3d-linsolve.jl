...
# Load the mesh explicitly rather than via `PointBoundary(path, m)`: `Orthtree`
...
cloud = WhatsThePoint.discretize(part, ConstantSpacing(Δ); alg = Orthtree(mesh))
